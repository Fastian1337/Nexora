    ...
    <div className="w-full max-w-md space-y-6 rounded-2xl border border-border/50 bg-card/40 p-8 backdrop-blur-xl shadow-2xl text-center">
      ...
      {loading ? (
        <div className="space-y-4">
          <div className="h-10 w-10 mx-auto animate-spin rounded-full border-4 border-t-blue-500 border-border/40" />
        ...
        <div className="space-y-6">
          ...
          <Link
            href="/login"
            className="inline-block px-6 py-2.5 border border-border/80 hover:bg-muted text-foreground font-semibold text-xs rounded-lg transition-all"
          >
            Back to login
          </Link>
        </div>
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-6 py-12">
      ...
      <React.Suspense fallback={
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground font-mono">Loading verification token...</p>
        </div>
      }>
        <VerifyEmailForm />
      </React.Suspense>
